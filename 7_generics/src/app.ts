...
// 'Array<T>'을 제네릭 타입이라고 한다. 즉, Array 타입과 T라는 타입과 연결된 타입을 말한다.
...
// 배열은 그 자체로 타입이지만 배열 내부에 특정 타입을 가진 데이터를 저장할 수 있다.
// 제네릭 타입 설정하기
// Array<string> === string[]
const names3: Array<string> = ["str"];
names3[0].split(" "); // 이 배열의 요소가 문자열 타입임을 추론하기 때문에 에러가 발생하지 않는다.

// 유니언 타입을 사용해 해당 배열은 문자열이나 숫자형을 저장하게 할 수 있다.
const names4: Array<string | number> = [];

// 프로미스 타입 : 프로미스라는 타입이 존재한다.
// 위 배열 타입과 같이 프로미스 타입도 다른 타입과 함께 작성될 수 있다.
...
/////// Generic Function ///////
// 제네릭 타입 선언
// 1) 함수의 매개변수 타입을 설정하는 것이다.
// 2) 타입의 첫 글자인 'T'를 입력하며 관례상 한 글자를 사용하는 것이 일반적이다.
// 3) 두 번째 제네릭 매개변수의 타입 이름은 알파벳 순서인 'U'를 사용하는 것이 일반적이다.
// 4) 제네릭이란 타입을 마치 함수의 파라미터처럼 사용하는 것을 말한다.
...
// 아래 함수는 T와 U의 인터섹션 타입을 반환함을 추론하고 있다.
// 제네릭 타입은 함수를 정의할 때 타입을 고정적으로 설정한 것이 아니라 함수를 호출할 때 동적으로 타입을 설정할 수 있도록 한다.
// 각 매개변수에 작성된 값을 타입으로 설정하여 유연하게 타입을 추론할 수 있다.
// objA와 objB의 타입명을 다르게 설정하였으므로 서로 다른 객체를 타입으로 함을 알 수 있다.
...
// 항상 Object.assign으로 인해 객체가 리턴되지만
// merge1 함수의 두 번째 매개변수로 숫자 33을 작성해도 에러가 발생하지 않는 문제가 생긴다.
// 모든 매개변수에 객체가 작성되도록 T와 U 타입의 제약 조건을 작성해야 한다.
// const mergedObj2 = merge1({ name: "Max" }, 33); // { name: 'Max' }

// 제약 조건 설정 : extends 키워드를 이용해 특정 타입을 갖도록 제약 조건을 설정한다.
...
// *** T 타입은 Lengthy 인터페이스를 상속하였으므로 매개변수 element는 length 속성(프로퍼티)가 포함되어 있어야 한다.
...
/////// "keyof" Constraint ///////
// 에러 발생 : 항상 obj 객체 내부에 매개변수인 key 키 속성을 갖는지 보장하기 어렵기 때문이다.
// function extractAndConvert1(obj: object, key: string) {
//   return obj[key];
// }

// obj는 객체를 타입으로 한 T 타입이며, key는 T 타입 객체의 키를 갖는 U 타입이다.
...
// 개선한 코드 : 제약 조건 설정하기
// T 타입은 string이나 number 또는 boolean 타입만을 갖도록 명시하여 위와 같이 객체를 작성하는 일을 피하도록 할 수 있다.
...
// 1. Partial 타입 : 특정 타입의 부분 집합을 만족하는 타입을 정의할 수 있다.
...
function createCourseGoal(
  ...
  // Partial<CourseGoal> : {}, {title: 'title'}, {description: 'description'}, {title: 'title', description: 'description', date: Date 타입을 가진 값} 등 CourseGoal 타입의 부분 집합을 모두 허용한다.
...
// 제네릭 타입과 유니언 타입의 공통점 : 둘 다 여러 타입을 동시에 다룬다.

// 제네릭 타입을 사용하는 경우
// 1) 특정 타입을 고정할 때
// 2) 전체 함수에 걸쳐 같은 타입을 사용하고자 할 때
// 3) 여러 타입에서 동작하는 함수를 생성하고자 할 때
// 3) 제네릭을 사용하지 않고 any 타입을 사용하면 여러 타입을 넣을 수 있지만 any는 타입 체크를 하지 않는다.
//    함수의 인자 타입과 반환 타입을 알 수 없다는 문제를 개선하고자 제네릭 타입을 사용한다.

// 유니언 타입을 사용하는 경우
// 1) 모든 메서드나 함수 호출마다 다른 타입을 지정하고자 할 때
