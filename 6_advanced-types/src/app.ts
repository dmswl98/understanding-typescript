...
// 두 타입(Admin, Employee)이 결합된 새로운 타입 생성 : '&'를 이용해 서로 다른 타입을 결합한다.
...
function printEmployeeInfo(emp: UnknownEmployee) {
  ...
  // 즉, if (typeof emp === "Employee")라고 작성할 수 없다.
...
// 구별된 유니언
// 1) 타입 가드를 쉽게 구현할 수 있게 하는 유니언 타입
// 2) 'type'이나 'kind' 프로퍼티에 타입 명을 작성하여 유니언을 구성하는 모든 타입 객체에는 공통 속성을 갖도록한다.
// 3) 인터페이스 내부에 작성된 속성은 사실상 타입 속성값이기 보다 리터럴 타입이다.
// 4) 주어진 속성의 존재 여부(in 연산자)를 확인하거나 instance 연산자를 사용하는 것이 아닌 실제 속성(공통 속성)을 확인해 어떤 타입인지 파악할 수 있다.
// 5) ide 자동 완성 기능이 존재해 오타의 위험을 줄여준다.
interface Bird {
  type: "bird";
  flyingSpeed: number;
}
...
/////// Type Casting ///////
// 형 변환(타입 캐스팅)
// 1) 타입스크립트가 직접 감지하지 못하는 특정 타입의 값을 타입스크립트에게 알리는 것을 말한다.
// 2) 화살표 괄호 내부에 타입을 명시하거나 as 구문을 사용해 타입을 명시할 수 있다.
// 3) 타입 캐스팅은 타입을 변환한다는 뜻이 아니라 타입스크립트에 특정 값이 특정 타입임을 알리기 위해 사용된다.
...
// 이를 해결하기 위해 해당 함수 선언문을 다시 작성해 해결할 수 있다.
// 함수 오버로드
// 1) 중괄호는 작성하지 않는다.
// 2) 해당 함수 상단에 함수 오버로드 코드를 작성한다.
// 3) 함수의 반환 타입을 정확히 추론하지 못하는 경우에 사용된다.
// 4) 타입스크립트는 함수의 정보와 함수의 코드를 하나로 병합하여 작동한다.
...
// 그러므로 문자열 메서드 코드에서 에러가 발생하지 않는다.
const result1 = add1("Max", "Tom");
result1.split(" ");

/////// Optional Chaining ///////
// 선택적 체이닝
// 1) 선택적 체이닝 연산자(?.)는 객체 데이터의 중첩된 속성과 객체에 안전하게 접근할 수 있도록 한다.
// 2) 선택적 체이닝 연산자(?.) 앞의 요소가 정의되지 않았다면 그 이후 요소에는 접근하지 않기 때문이다.
// 3) 백엔드에서 접근하고자 하는 데이터의 존재 여부가 확실치 않은 요소에 사용한다.
...
// 널 병합
// 1) 널 병합 연산자 : ??
// 2) 연산자 왼쪽 항이 null이거나 undefined라면 오른쪽 항을 반환한다.
// 3) 널 병합 연산자를 사용하면 피연산자 중 '값이 할당된' 변수를 빠르게 찾을 수 있다.
// 4) 빈 문자열('')과 0은 truthy value로 취급한다.
const input1 = "";
const input2 = "hi";
