...
// 데코레이터
// 1) 데코레이터는 클래스, 프로퍼티(속성), 메서드, 접근 제어자, 매개변수 등에 사용할 수 있는 특별한 함수이다.
// 2) 선언된 데코레이터 함수를 사용할 때는 데코레이터 이름 앞에 @를 붙인다.
// 3) 데코레이터는 클래스를 정의할 때 실행되며, 클래스 내부의 메서드를 실행하거나 프로퍼티에 접근할 때 작동되지 않는다.
// 4) 즉, 데코레이터가 설정된 클래스로 한 인스턴스를 생성하지 않더라도 데코레이터는 작동한다.

/////// Class Decorator ///////
// 클래스 데코레이터
// 1) 라이브러리 내 데코레이터명은 대부분 대문자로 시작하므로 첫 글자를 대문자로 작성한다.
// 2) 설정된 클래스 선언을 관찰, 수정 또는 대체하는데 사용한다.
// 3) 클래스 데코레이터는 런타임에 함수로 호출된다.
// 4) 데코레이팅되는 클래스가 유일한 인자로 전달되어 호출된다.
// 5) 클래스 데코레이터는 클래스에 해당하는 인스턴스가 생성되지 않고 클래스가 정의만 되어도 실행된다.
...
/////// Decorator Factories ///////
// 데코레이터 팩토리
// 1) 데코레이터 함수에서 팩토리를 사용해 사용자로부터 인자를 전달받을 수 있다.
// 2) 데코레이터 팩토리 함수는 '데코레이터 함수'를 감싸는 래퍼 함수이다.
// 3) 'Logger1()'는 데코레이터 팩토리 함수를 실행하는 것이 아니라 데코레이터 팩토리 함수가 반환하는 데코레이터 함수를 실행하는 것이다.
...
// 속성(프로퍼티) 데코레이터
// 1) 특정 속성 선언 앞에서 사용된다.
// 2) 속성 선언을 확인, 수정, 교체하는데 사용된다.
// 3) 속성 데코레이터 함수는 전달받는 인자가 2개이며 다음과 같다.
//   1. target: any
//   2. prop: string
...
// 접근 제어자 데코레이터
// 1) 접근 제어자 앞에 사용된다.
// 2) 접근 제어자에 대한 선언 확인, 수정, 교체하는데 사용된다.
// 3) 접근 제어자 데코레이터 함수는 전달받는 인자가 3개이며 다음과 같다.
//    1. target: any
//    2. prop: string
...
// 메서드 데코레이터
// 1) 메서드 선언 앞에 사용된다.
// 2) 메서드 선언을 확인, 수정, 교체하는데 사용된다.
// 3) 메서드 데코레이터 함수는 전달받는 인자가 3개이며 다음과 같다.
//    1. target: any
//    2. prop: string
//    3. descriptor: PropertyDescriptor
function Log3(
...
// 파라미터(매개변수) 데코레이터
// 1) 클래스 생성자 함수 또는 메서드의 매개변수에 사용된다.
// 2) 매개변수 데코레이터 함수는 전달받는 인자가 3개이며 다음과 같다.
//    1. target: any
//    2. prop: string
...
// 이제껏 작성한 데코레이터 함수는 어떠한 것도 반환하지 않았다.
// 데코레이터 팩토리 함수는 데코레이터 함수를 반환하지만 데코레이터 함수는 아무것도 반환하지 않았다.
// 데코레이터 함수에서 반환 값을 추가할 수 있다.

// 클래스 데코레이터 함수의 클래스 반환
// 1) 반환된 클래스의 내부 코드는 클래스가 정의될 때 수행되지 않고 데코레이터를 설정한 클래스의 인스턴스를 생성한 경우에 실행된다.
// 2) 인스턴스를 생성하지 않으면 화면에 클래스 필드 name의 값이 랜더링되지 않는다.
// 3) 클래스를 반환함으로써 데코레이터가 설정된 클래스를 대체 및 확장할 수 있다.
